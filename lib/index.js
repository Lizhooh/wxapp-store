...
module.exports = class Store {
    ...
    _keys(exp) {
        if (this._opt.async) {
            return new Promise((resolve, reject) => {
                this.wx.getStorageInfo({
                    success: info => {
                        if (isType(exp) === 'RegExp' && Array.isArray(info.keys)) {
                            resolve(info.keys.filter((key, index) => isType(exp) === 'RegExp' && exp.test(key)));
                        }
                        resolve(info.keys);
                    },
                    fail: err => reject(err),
            ...
            const info = this.wx.getStorageInfoSync();
            if (isType(exp) === 'RegExp' && Array.isArray(info.keys)) {
                info.keys.filter((key, index) => {
                    if (isType(exp) === 'RegExp') {
                        return exp.test(key)
                    }
                    return false;
                });
    ...
    keys(exp) {
        try {
            return this._keys(exp);
        }
        catch (e) {
            console.error('Store keys error:', e);
        }
    }
}
